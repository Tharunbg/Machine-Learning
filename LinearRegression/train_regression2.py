...
y = load_data.data[:, 0:1]  # Sepal length
...
print()
plt.plot(loss_value)
plt.title('Loss vs Steps of Petal length, petal width -> Sepal length and Calculating error without regularization')
plt.xlabel('Steps')
plt.ylabel('Loss')
...
print()
plt.plot(loss_value)
plt.title('Loss vs Steps of Petal length, petal width -> Sepal length and Calculating error with regularization = 0.2')
plt.xlabel('Steps')
plt.ylabel('Loss')
plt.savefig('train_regression1_loss.png')
plt.show()
 
model.save_weight_bias('regression2_output_model.npz')